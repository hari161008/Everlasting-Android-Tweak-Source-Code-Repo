...
class AutoFreezeWorker(context: Context, params: WorkerParameters) : Worker(context, params) {
    ...
    private fun isSkipApp(context: Context, appInfo: AppInfo): Boolean =
        ...
        )) || (HailData.skipNotifyingApp && AutoFreezeService.isInstanceInitialized &&
            try { AutoFreezeService.instance.activeNotifications?.any { it.packageName == appInfo.packageName } ?: false } catch (_: Exception) { false }) || appInfo.whitelisted
}