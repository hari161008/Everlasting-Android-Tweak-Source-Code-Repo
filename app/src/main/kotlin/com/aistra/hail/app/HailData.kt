...
object HailData {
    ...
    const val URL_FDROID = "https://f-droid.org/packages/com.coolappstore.everlastingandroidtweak"
    ...
    val VERSION get() = "${com.coolappstore.everlastingandroidtweak.BuildConfig.VERSION_NAME} (${com.coolappstore.everlastingandroidtweak.BuildConfig.VERSION_CODE})"
    ...
    val WORKING_MODE_VALUES = listOf(
        MODE_DEFAULT, MODE_SHIZUKU_STOP, MODE_SHIZUKU_DISABLE, MODE_SHIZUKU_HIDE,
        MODE_SHIZUKU_SUSPEND, MODE_SU_STOP, MODE_SU_DISABLE, MODE_SU_HIDE, MODE_SU_SUSPEND,
        MODE_DHIZUKU_HIDE, MODE_DHIZUKU_SUSPEND, MODE_OWNER_HIDE, MODE_OWNER_SUSPEND,
        MODE_ISLAND_HIDE, MODE_ISLAND_SUSPEND, MODE_PRIVAPP_STOP, MODE_PRIVAPP_DISABLE
    )
    ...
    // ── Everlasting ↔ Hail sync keys (written by Everlasting, read by Hail) ──
    // Stored in default SharedPreferences so both sides can access them
    // synchronously without DataStore.
    //
    // EVERLASTING_THEME_MODE: 0=system, 1=light, 2=dark, 3=pure black, 4=pure white
    //   -1 means Everlasting has not written the value yet (first launch).
    const val EVERLASTING_THEME_MODE    = "everlasting_theme_mode"
    const val EVERLASTING_DYNAMIC_COLOR = "everlasting_dynamic_color"
    // Custom primary colour hex string (e.g. "FF6200EE"). Empty string = use scheme default.
    // Written by Everlasting when the user picks a custom accent in Settings.
    const val EVERLASTING_CUSTOM_PRIMARY = "everlasting_custom_primary"
    // Tab-pills in the App Freeze home screen.
    // true = solid/opaque surface  |  false = semi-transparent frosted glass
    const val FREEZE_TAB_PILL_SOLID     = "freeze_tab_pill_solid"
    // Home-screen category-pill style (synced from Everlasting DataStore → SharedPrefs)
    // 0 = Match App UI  |  1 = Blur  |  2 = System default
    const val HOME_PILL_STYLE           = "home_pill_style"
    const val HOME_PILL_BLUR_INTENSITY  = "home_pill_blur_intensity"
    // ──────────────────────────────────────────────────────────────────────────
    ...
    val TILE_ACTION_VALUES = listOf(
        AUTO_FREEZE_AFTER_LOCK, ACTION_FREEZE_ALL, ACTION_UNFREEZE_ALL,
        ACTION_FREEZE_NON_WHITELISTED, ACTION_LOCK, ACTION_LOCK_FREEZE
    )
    ...
    val DYNAMIC_SHORTCUT_ACTIONS = listOf(
        ACTION_NONE, ACTION_FREEZE_ALL, ACTION_UNFREEZE_ALL,
        ACTION_FREEZE_NON_WHITELISTED, ACTION_LOCK, ACTION_LOCK_FREEZE
    )

    // 'sp' is private — Everlasting writes sync keys via its own PreferenceManager call,
    ...
    // Everlasting-sync reads (synchronous via SharedPreferences)
    val everlastingThemeMode     get() = sp.getInt(EVERLASTING_THEME_MODE, -1)
    val everlastingDynamicColor  get() = sp.getBoolean(EVERLASTING_DYNAMIC_COLOR, true)
    /** Hex ARGB string of custom accent, e.g. "FF6200EE". Empty = use scheme default. */
    val everlastingCustomPrimary get() = sp.getString(EVERLASTING_CUSTOM_PRIMARY, "") ?: ""
    val freezeTabPillSolid       get() = sp.getBoolean(FREEZE_TAB_PILL_SOLID, true)
    /** 0=Match App UI, 1=Blur, 2=System default */
    val homePillStyle           get() = sp.getInt(HOME_PILL_STYLE, 0)
    val homePillBlurIntensity   get() = sp.getFloat(HOME_PILL_BLUR_INTENSITY, 16f)
