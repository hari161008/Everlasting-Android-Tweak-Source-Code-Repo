...
class ZeroPaddingAppWidgetHostView(
    ...
    fun canHaveDisplayListCompat(): Boolean {
        context.logUtils.debugLog("canHaveDisplayList() ${this::class.java.name}")
        return try {
            val m = View::class.java.getDeclaredMethod("canHaveDisplayList")
            m.isAccessible = true
    ...
    fun onDefaultViewClickedCompat(view: View) {
        try {
            val m = context.appWidgetManager.javaClass.getMethod("noteAppWidgetTapped", Int::class.java)
            m.invoke(context.appWidgetManager, appWidgetId)
        } catch (_: Exception) {}
