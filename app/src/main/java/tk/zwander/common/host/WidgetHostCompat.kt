...
class WidgetHostCompat(
    ...
    private fun createOnClickHandlerForWidget(widgetId: Int): Any {
        return when (mode) {
            ...
            is Mode.Fallback -> {
                // No click interception available on this ROM — return a no-op object
                object : Any() {}
            }
        }
    }
    ...
    override fun onCreateView(
        ...
        if (mode !is Mode.Fallback) {
            try {
                ...
                context.logUtils.normalLog("Unable to set click handler field for widget $appWidgetId", e)
            }
        }
    ...
    sealed class Mode(val handlerClass: java.lang.Class<*>) {
        ...
        // Used when neither handler class exists on this ROM — clicks pass through normally
        class Fallback : Mode(Any::class.java)
    }
}