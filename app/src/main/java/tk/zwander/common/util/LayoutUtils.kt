...
fun ComponentActivity.setThemedContent(content: @Composable () -> Unit) {
    // Sync the Activity's night mode so XML-inflated Views also follow Everlasting's theme setting
    val sp = androidx.preference.PreferenceManager.getDefaultSharedPreferences(this)
    val evMode = sp.getInt(com.aistra.hail.app.HailData.EVERLASTING_THEME_MODE, -1)
