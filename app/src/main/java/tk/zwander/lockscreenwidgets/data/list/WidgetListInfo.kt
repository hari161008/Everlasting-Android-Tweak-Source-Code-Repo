...
private fun userHandleId(handle: UserHandle): Int = try {
    UserHandle::class.java.getDeclaredMethod("getIdentifier")
        .also { it.isAccessible = true }.invoke(handle) as Int
} catch (_: Exception) { handle.hashCode() }
    ...
    override fun compareTo(other: BaseListInfo<AppWidgetProviderInfo>): Int {
        ...
        return if (nameResult == 0) {
            ...
            if (widthResult == 0) {
                ...
                if (heightResult == 0) {
                    userHandleId(itemInfo.profile).compareTo(userHandleId(other.itemInfo.profile))
                } else {
                    heightResult
                }
            } else {
